...
- フォント差分でユーザーがレビュー画面を基準に指定した場合、比較対象は常にそのレビュー画像/Studio表示に固定する。過去の書き出しや「正しいはずのフォント」を基準にしない。
- StudioをWindows Chromeで見ていて、Remotion書き出しをWSL Chromiumで実行すると、同じCSSでもフォント解決とラスタライズが変わる。完全一致が必要な場合は、レビューと同じWindows Chrome経路でstill確認してからMP4を書き出す。
- Regular単体フォントに `font-synthesis: none` を入れると、レビュー側で見えていたブラウザの太字描画より細くなることがある。レビューが合成太字を含む見た目なら、同じウェイト解決を再現する。
